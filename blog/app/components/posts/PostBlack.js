  ...
  return (
    <article className="post-card post-category-top group relative has-line-link-white">
      <div className="relative">
        <div
          className={`text-white bg-${color} z-10 absolute right-2 top-2 border border-white px-2 py-1 capitalize rounded-md hover:opacity-70 `}
        >
          <LocalizedLink
            href={`/category/${slugify(category)}`}
            className={`transition duration-300 bg-${color} text-white border-white/30 `}
          >
            {category}
          </LocalizedLink>
        </div>
